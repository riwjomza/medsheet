  ...
  return (
    <div className={`bg-white w-fit p-6 rounded-2xl flex flex-col gap-3 text-center drop-shadow-lg`}>
      ...
      <form action="" className={`flex flex-col gap-3 text-center`}>
        <div>
          <input type="text" value={email} onChange={(e) => setEmail(e.target.value)}  placeholder='username' name="" id="" className='border w-full p-2 max-w-[35ch] rounded-md border-black-light '/>
        </div>
        <div>
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}  placeholder='password' name="" id="" className='border w-full p-2 rounded-md border-black-light '/>
        </div>
        <div>
          <button onClick={submitHandler} className={`bg-purple-light text-white px-4 py-1 rounded-sm`}>SIGN IN</button>
        </div>
        <div>
          <h2 className='duration-300 hasdover:scale-110 cursor-pointer' onClick={() => setIslogginIn(!isLoggingIn)}>{!isLoggingIn ? 'LOGIN' : 'Register'}</h2>
        </div>
      </form>
    </div>
  )
